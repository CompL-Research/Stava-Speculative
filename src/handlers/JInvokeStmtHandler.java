...
// 		}
		...
		if (expr instanceof InstanceInvokeExpr) {
			...
			for(Edge edge: edges) {
				// Edge edge = edges.next();
				ConditionalValue cv = new ConditionalValue(edge.tgt(), new ObjectNode(-1, ObjectType.parameter), true, utils.getBCI.get(edge.srcUnit()));
				ptg.cascadeCV((Local) base, cv, summary, m);
			}
		}
		else if (expr instanceof JStaticInvokeExpr) {
		...
		for(Edge edge: edges) {
			...
			for (int i = 0; i < paramCount; i++) {
				...
				ConditionalValue cv = new ConditionalValue(method, obj, true, utils.getBCI.get(edge.srcUnit()));
				
				if (edge.kind() == Kind.REFL_INVOKE)
					ptg.cascadeCV((Local) args.get(1), cv, summary, m);
				else if(edge.kind() == Kind.REFL_CONSTR_NEWINSTANCE)
					ptg.cascadeCV((Local) args.get(0), cv, summary, m);
				else {
					...
					if (arg.getType() instanceof RefType || arg.getType() instanceof ArrayType) {
						if (!(arg instanceof Constant)) {        // Notice the not(!)
							if (isNative && !iswhiteListed) {
								...
								ptg.cascadeCV((Local) args.get(i), cv, summary, m);
						}
					}
				}
			}
